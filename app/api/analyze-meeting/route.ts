  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `You are an expert project management assistant. Analyze the following meeting transcript and extract structured information.
...
{
  "meeting_metadata": {
    "project_name": "string (extract from context)",
    "date": "string (extract if mentioned, otherwise 'Not specified')",
    "attendees": ["array of attendee names if mentioned"]
  },
  "action_items": [
    {
      "task": "string",
...
Be thorough but concise. If information isn't present, use "Not specified" or empty arrays as appropriate.
...
Remember: Respond with ONLY the JSON object, nothing else.`,
    ...
    // Strip markdown code blocks if present (more aggressive)
